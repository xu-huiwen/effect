...
if(document.getElementById('lotteryBox8')) {
    //开始抽奖flash->js
    window.callJsToStart8 = function(){
        //alert("抽奖失败");
        //SWFOBJ.reset();
        callFlashToRoll8(1);
        //PTTSendClick('lottery','start','开始抽奖');
    }
//开发获得抽奖结果 通知flash开始播放效果 js->flash
    window.callFlashToRoll8 = function(id){
        //通知转盘转到对应的中奖产品的id （序号从0,1,2.....沿着效果光走的顺序，对应下面配置中奖品光效的位置 如id=0 对应显示在19_20的位置）
        if(SWFOBJ8)SWFOBJ8.stopRoll(id);
        PTTSendClick('lottery','award'+id,'奖品');
    }
//3、flash动画完成通知js  flash->js
    window.callJsToComplete8 = function(){
        PTTSendClick('lottery','complete','抽奖完成');
        alert('恭喜你获得大奖哟！！');
    }

    window.SWFOBJ8= new Lottery({
        'lighturl':'http://game.gtimg.cn/images/codo/2017/atemple/img/sel.png',//外部光圈png  不填写就用默认的效果
        'width':822,//flash 宽度
        'height':386,//flash 高度
        'total':8,//抽奖产品的总数
        'sbtnx':274,// 开始抽奖按钮的位置x坐标
        'sbtny':128 ,// 开始抽奖按钮的位置y坐标
        'sbtnw':275,// 开始抽奖按钮的宽度
        'sbtnh':129,// 开始抽奖按钮的高度
        'boxw':273,// 奖品光效的宽度
        'boxh':127,//奖品光效的高度
        'position':"1_1,275_1,548_1,1_129,548_129,1_257,275_257,548_257",
        'contentId' : 'lotteryBox8',
        'onClickRollEvent' : callJsToStart8,
        'onCompleteRollEvent':callJsToComplete8
    });
}
