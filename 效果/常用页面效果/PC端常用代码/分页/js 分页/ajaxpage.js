...
var AjaxPage = function(opt) {
    ...
    this.linkFirst = function() {
        return '<a href="javascript:' + this.option.oPage + '.to(1);" class="pagepre pagefirst">首页</a>'
    };
    //上一页
    this.linkFore = function(i) {
        return '<a href="javascript:' + this.option.oPage + '.to(' + i + ');" class="pagepre previouspage"  >上一页</a>'
    };
    this.linkForeNone = function() {
        return '<a href="javascript:;" class="pageprenone previouspageone" title="当前已经是第一页" >上一页</a>'
    };
    //显示的数字
    this.linkNum = function(i) {
        if (i * 1 == this.option.pageNow * 1) {
            return '<span class="pageprenone pagenow">' + i + '</span>';
        }
        return '<a href="javascript:' + this.option.oPage + '.to(' + i + ');" class="pagepre" >' + i + '</a>';
    };
    ...
    this.linkNext = function(i) {
        return '<a href="javascript:' + this.option.oPage + '.to(' + i + ');" class="pagenext" >下一页</a>'
    };
    this.linkNextNone = function() {
        return '<a href="javascript:;" class="pagenextnone" title=" 当前已经是最后一页">下一页</a>'
    };
    //尾页
    this.linkLast = function() {
        return '<a href="javascript:' + this.option.oPage + '.to(' + this.option.pageTotal + ');" class="pagenext">尾页</a>'
    };
    //跳转页面
    this.to = function(i) {
        if (isNaN(i) || i <= 1) {
    ...
    this.makeList = function(style) {
        ...
        //输入框
        pageList[7] = ' 第<input size="4" onkeydown="if(event.keyCode==13) ' + this.option.oPage + '.to(this.value)" value="' + pageNow + '" style="width:20px">页 <a href="javascript:" onclick="' + this.option.oPage + '.to(' + _v + ');" style="cursor:pointer" align="absmiddle">GO</a>';
