var getFlashVersion = function() {
	...
	if (document.all) {
		...
		if (swf) {
			...
			flashVersion = VSwf.split(" ")[1].split(",");
		}
	} else {
...
var setHeight = function() {
	...
	if (needHmin && height < needHmin) {
		height = needHmin
	}
	if (needHmax && height > needHmax) {
		height = needHmax
	}
...
var getSwfStr = function(swfid, url, width, height, flashvars, params) {
	...
	for (var i in opt) {
		v += '<param name="' + i + '" value="' + this.opt[i] + '" />';
	}
...
function insertSwfV2(elm, url, width, height, flashvars, params, version) {
	...
	if (isVer) {
		for (var i = 0; i < ver.length; i++) {
			if (parseInt(fver.v[i]) < parseInt(ver[i])) {
				isVer = false;
				break
			}
		}
